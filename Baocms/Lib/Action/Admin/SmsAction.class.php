...
class SmsAction extends CommonAction {
    ...
    public function create() {
        if ($this->isPost()) {
            ...
            $obj = D('Sms');
            if ($obj->add($data)) {
                $this->niuSuccess('添加成功', U('sms/index'));
            }
            $obj->cleanCache();
            $this->niuError('操作失败！');
        } else {
        ...
        if ($sms_id = (int) $sms_id) {
            ...
            if ($this->isPost()) {
                ...
                if (false !== $obj->save($data)) {
                    $obj->cleanCache();
                    $this->niuSuccess('操作成功', U('sms/index'));
                }
    ...
    public function delete($sms_id = 0) {
        if (is_numeric($sms_id) && ($sms_id = (int) $sms_id)) {
            ...
            $obj->cleanCache();
            $this->niuSuccess('关闭成功！', U('sms/index'));
        } else {
    ...
    public function audit($sms_id = 0) {
        if (is_numeric($sms_id) && ($sms_id = (int) $sms_id)) {
            ...
            $obj->cleanCache();
            $this->niuSuccess('开启成功！', U('sms/index'));
        } else {
            $sms_id = $this->_post('sms_id', false);
            if (is_array($sms_id)) {
                $obj = D('Sms');
