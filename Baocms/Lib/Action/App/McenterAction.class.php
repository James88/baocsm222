	...
	class  McenterAction extends  CommonAction{
		...
		public function tuan()
		{
			if(!$uid = $this->_param('uid')){
				...
				$start = ($page-1)*$limit;
				$order_list = $Tuanorder->where($map)->order(array('order_id' => 'desc'))->limit($start.','.$limit)->select();
				$data = array('status'=>self::BAO_REQUEST_SUCCESS,'order_list'=>$order_list,'page'=>$page+1);
		...
		public function tuan_detail(){
			...
			if(!$uid = $this->_param('uid')){
				...
				$data = array('status'=>self::BAO_REQUEST_SUCCESS,'detail'=>$detail,'tuans'=>$tuans);
			}
			$this->stringify($data);
        }
		...
		public function faces() {
			if(!$uid = $this->_param('uid')){
				...
				if($face = htmlspecialchars($this->_param('face'))){
					if(!isImage($face)){
						$data = array('status' => self::BAO_FROM_FALSE, 'msg'=>"头像格式不正确");
					}else{
						echo $face;echo "File:", __FILE__, ',Line:',__LINE__;exit;
						$datas = array('user_id' => $uid, 'face' => $face);
						if (false !== D('Users')->save($datas)) {
							$data = array('status'=>self::BAO_REQUEST_SUCCESS ,'msg'=>'昵称设置成功！!','url'=>U('mcenter/face',array('uid'=>$uid)));
						}else{
